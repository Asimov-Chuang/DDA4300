"""Approach 1 experiments for online resource allocation in auction markets.
...
Run from the project root:
    python approach1/approach1_experiment.py
"""
...
def make_plots(
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(result_dir / "approach1_revenue_ratio.png", dpi=200)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(result_dir / "approach1_resource_usage.png", dpi=200)
    plt.close(fig)
...
def main() -> None:
    ...
    print_table(rows)
    write_csv(rows, result_dir / "approach1_results.csv")

    np.savez_compressed(
        result_dir / "approach1_workspace.npz",
        requests=requests,
        bids=bids,
        capacities=capacities,
        pbar=pbar,
